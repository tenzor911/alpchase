    ...
    if( $country_id > 0 ) {			
        foreach( $services as $key => $value ) {				
            ...
            if( isset( $services[$country_key][$key] ) ) $podservice_id = intval( $podservices[$country_key][$key] ); 
                ...
                $podservice_id = 0;			
            if( $service_id > 0 ) {	
                mysql_query( "INSERT INTO `order_basket` (`customer_id`, `country_id`, `service_id`, `podservice_id`) VALUES ('{$c_id}', '{$country_id}', '{$service_id}', '{$podservice_id}')" );				
                echo "INSERT INTO `order_basket` (`customer_id`, `country_id`, `service_id`, `podservice_id`) VALUES ('{$c_id}', '{$country_id}', '{$service_id}', '{$podservice_id}')<br>";			
            }		
        }	
    }
}
